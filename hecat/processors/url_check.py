...
def check_return_code(url, current_item_index, total_item_count, errors):
    try:
        ...
        if response.status_code in VALID_HTTP_CODES:
            ...
            return False
    except (requests.exceptions.ConnectionError, requests.exceptions.ReadTimeout, requests.exceptions.ContentDecodingError, requests.exceptions.TooManyRedirects, requests.exceptions.InvalidURL) as connection_error:
