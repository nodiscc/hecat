...
test_import_awesome_selfhosted: install
	...
	.venv/bin/hecat --config tests/.hecat.import_awesome_selfhosted.yml && \
	.venv/bin/hecat --config tests/.hecat.import_awesome_selfhosted_nonfree.yml
...
test_url_check: install
	.venv/bin/hecat --config tests/.hecat.url_check.yml
...
test_update_software_metadata: install
	.venv/bin/hecat --log-level DEBUG --config tests/.hecat.update_metadata.yml

.PHONY: test_awesome_lint # test linter/compliance checker on awesome-sefhosted-data
test_awesome_lint: install
	.venv/bin/hecat --config tests/.hecat.awesome_lint.yml
...
test_export_awesome_selfhosted_md: install
	.venv/bin/hecat --config tests/.hecat.export_markdown_singlepage.yml && \
...
test_export_awesome_selfhosted_html: install
	...
	.venv/bin/hecat --config tests/.hecat.export_markdown_multipage.yml && \
	...
	.venv/bin/sphinx-build -b html -c tests/ -d tests/awesome-selfhosted-html/.doctrees tests/awesome-selfhosted-html/md/ tests/awesome-selfhosted-html/html/
...
test_import_shaarli: install
	.venv/bin/hecat --config tests/.hecat.import_shaarli.yml
...
test_download_video: install
	...
	.venv/bin/hecat --log-file tests/hecat.log --config tests/.hecat.download_video.yml
...
test_download_audio: install
	.venv/bin/hecat --config tests/.hecat.download_audio.yml
...
test_archive_webpages: install
	...
	.venv/bin/hecat --log-level DEBUG --config tests/.hecat.archive_webpages.yml
...
test_export_html_table: install
	...
	.venv/bin/hecat --config tests/.hecat.export_html_table.yml

TRIVY_VERSION=0.44.0
TRIVY_EXIT_CODE=1
.PHONY: scan_trivy # run trivy vulnerability scanner
scan_trivy:
	.venv/bin/pip3 freeze --local > tests/requirements.txt
